...
class ResNet1d(nn.Module):
    def __init__(self, block, layers, input_channels=12, inplanes=64, num_classes=9):
        ...
        self.fc = nn.Linear(512 * block.expansion * 2, 30)
    ...
    def forward(self, x):
        ...
        x = self.fc(x)
        hid_emb = x
        return hid_emb, self.fc2(x)


class ResNet1d_MHL(nn.Module):
    def __init__(self, block, layers, input_channels=12, inplanes=64):
...
class ResNet1d_LSTM(nn.Module):
    def __init__(self, block, layers, input_channels=12, inplanes=64, num_classes=9):
        ...
        self.dropout = nn.Dropout(0.2)
        self.time_steps = 50
        self.lstm_output = 512 * block.expansion * 2
        self.lstm = nn.LSTM(input_size=self.lstm_output,
                            hidden_size=self.lstm_output,
                            num_layers=1,
                            batch_first=True,
                            bidirectional=False)
...
def resnet34_MHL(**kwargs):
    model = ResNet1d_MHL(BasicBlock1d, [3, 4, 6, 3], **kwargs)
    return model

def resnet34_LSTM(**kwargs):
    model = ResNet1d_LSTM(BasicBlock1d, [3, 4, 6, 3], **kwargs)
    return model